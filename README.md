...
- **Pools this page cannot measure** — pools that minted blocks but offered no address to probe, so no axis could be measured. Listed unscored and unranked, and findable through the same search box.
...
- **Being unmeasurable is not a finding.** Blocks can be produced and propagated without a registered
  relay. An operator may leave relays unregistered to limit exposure to denial-of-service or for
  privacy, may use a third-party relay service registered under another name, or may have an ordinary
  configuration reason. From outside the chain these cannot be told apart.
...
- `tools/template.html` holds all markup, styling and the i18n dictionary. It is the single source
  for the presentation layer; the script only substitutes `__DATA__`, `__GROUPS__`, `__UNMEASURED__`
  and `__CHECKED__`.
